...
pub struct InterfaceInner {
    ...
    ip_addrs_listen: Vec<IpCidr, IFACE_MAX_ADDR_COUNT>,
}
...
pub struct Config {
    ...
    /// If this variable is true hardware_addr is checked for ethernet packet
    pub hardware_addr_check: bool,
...
impl Config {
    pub fn new(hardware_addr: HardwareAddress, hardware_addr_check : bool) -> Self {
        Config {
            random_seed: 0,
            hardware_addr,
            hardware_addr_check,
...
impl Interface {
    ...
    pub fn new(config: Config, device: &mut (impl Device + ?Sized), now: Instant) -> Self {
        ...
        Interface {
            ...
            inner: InterfaceInner {
                ...
                hardware_addr_check : config.hardware_addr_check,
                ip_addrs: Vec::new(),
                any_ip: false,
                routes: Routes::new(),
                #[cfg(any(feature = "medium-ethernet", feature = "medium-ieee802154"))]
                neighbor_cache: NeighborCache::new(),
                ip_addrs_listen: Vec::new(),
                #[cfg(feature = "multicast")]
    ...
    /// # Panics
    /// This function panics if any of the addresses are not unicast.
    pub fn update_ip_addrs_listen<F: FnOnce(&mut Vec<IpCidr, IFACE_MAX_ADDR_COUNT>)>(&mut self, f: F) {
        f(&mut self.inner.ip_addrs_listen);
        InterfaceInner::flush_neighbor_cache(&mut self.inner);
        InterfaceInner::check_ip_addrs(&self.inner.ip_addrs_listen);
    }
...
impl InterfaceInner {
    ...
    /// Check whether the interface has the given IP address assigned.
    fn has_ip_addr_listen<T: Into<IpAddress>>(&self, addr: T) -> bool {
        let addr = addr.into();
        self.ip_addrs_listen.iter().any(|probe| probe.address() == addr)
    }
